$(function() {
  $('.public_image').change(function() {
    if (user.likes < site_data.wall_likes) {
      fireNotification({msg: 'يحتاجلك لأيكات حبي ' + site_data.wall_likes + 'واعجابات '});
      return false;
    }
    ...
    $('.public_from').submit(function(e) {
      ...
      if (user.likes < site_data.wall_likes) {
        $('.public_message').val('');
        fireNotification({msg: 'الكتابة في الحائط تتطلب منك الحصول علي ' + site_data.wall_likes + 'إعجاب'});
        return false;
      }
      if (user.last_post == undefined || user.last_post == 0) {
    ...
    socket.on('appendWall', function(data) {
      ...
      var d = new Date();
      var n = d.getTime();
      if ($('#public').hasClass('slideshow') == false) {
        wall_count += 1;
      ...
      if (data.status == 0) {
        ...
        if (data.post.type == "image") {
          msg = "<button onclick='fileBtn(" + data.post.id + ")' file-id=" + data.post.id + " class='btn-file-" + data.post.id + " btn fa fa-image'>عرض الصوره</button> <a href='" + base_url + "/images/wall/" + data.post.image + "' target='_blank' > <img style='display:none;max-width: 200px;max-height: 200px;' class='img-responsive post-" + data.post.id + "' src='" + base_url + "/images/wall/" + data.post.image + "'></a>";
        } else if (data.post.type == "video") {
          msg = "<button onclick='fileBtn(" + data.post.id + ")' file-id=" + data.post.id + " class='btn-file-" + data.post.id + " btn fa fa-youtube-play'>عرض الفيديو</button> <video class='post-" + data.post.id + "' style='display:none; width:90%;max-height: 200px;' controls><source src='" + base_url + "/images/wall/" + data.post.image + "' type='video/mp4'></video>";
        } else if (data.post.type == "audio") {
          msg = "<button onclick='fileBtn(" + data.post.id + ")' file-id=" + data.post.id + " class='btn-file-" + data.post.id + " btn fa fa-youtube-play'>مقطع الصوت</button><audio class='post-" + data.post.id + "' style='display:none; width: 90%; width:200px;' controls><source src='" + base_url + "/images/wall/" + data.post.image + "' type='audio/mpeg'></audio>";
          ...
          "</button>" +
          "<iframe style='width: 99%;height: 205px;display:none;max-width: 240px;' class='post-" + data.post.id + "'></iframe>";
        } else {
        ...
        if (msg != "") {
          ...
          var display = (user.type == "admin" || user.group.delete_wall == 1 || user.id == data.post.user_id) ? 'display:block' : 'display:none';
